...
class XUser:
    def __init__(self, img):
        self.UserX = pygame.transform.scale(pygame.image.load(img).convert_alpha(),(45, 35))
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    while run:
                        USERCHOICE = "Bird"
                        THEMECHOICE = "Sky"
                        MainGame(USERCHOICE, THEMECHOICE)
...
def WelcomePage():
    ...
    while True:
        ...
        screen.blit(HIGHSCORE, (SCREENWIDTH -  175, 10)) 
...
def MainGame(USERCHOICE, THEMECHOICE):
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and IS_ACTIVE == False:
                    ...
                    PLAYER_MOVEMENT = 0
                    SCORE = 0
                if event.key == pygame.K_ESCAPE and IS_ACTIVE == False:
                    IS_ACTIVE = True
                    GreenPipeList.clear()
                    StarList.clear()
                    UserRect.center = (100,325)
        ...
        if IS_ACTIVE:
            ...
            DText('Game Over', SCREENWIDTH * 0.5, SCREENHEIGHT * 0.25, MEDUIMBLUE)
            DText('Your Score: ' + str(SCORE), SCREENWIDTH * 0.5, SCREENHEIGHT * 0.375, MEDUIMBLUE)
            DText('Use "space" key to Replay or "esc" key to return Home', SCREENWIDTH * 0.5, SCREENHEIGHT * 0.5, MEDUIMBLUE)
...
def GameMenu():
    ...
    while True:
        ...
        UNTextBorder = pygame.Rect(((SCREENWIDTH - UserNameText.get_width()) / 2) - 5, SCREENHEIGHT * .20, UserNameText.get_width() + 10, 40)
        ...
        if UNameActive:
            ...
            USERNAMEPrompt = SmallFont.render("Enter UserName here", True, MEDUIMBLUE)
        else:
...
def SuccessScreen(USERNAME, USERCHOICE, THEMECHOICE):
    ...
    while True:
        ...
        DText("Hello, " + USERNAME + ".", SCREENWIDTH * 0.5, SCREENHEIGHT * 0.25, NAVYBLUE)
        DText("Your User Choice is: " + USERCHOICE + ".", SCREENWIDTH * 0.5, SCREENHEIGHT * 0.375, NAVYBLUE)
        DText("Your Theme Choice is: " + THEMECHOICE + ".", SCREENWIDTH * 0.5, SCREENHEIGHT * 0.5, NAVYBLUE)
        DText("Press 'space' key to play game or 'esc' key to return Home.", SCREENWIDTH * 0.5, SCREENHEIGHT * 0.625, NAVYBLUE)
